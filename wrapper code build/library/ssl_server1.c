...
#endif
/*
#define SERVER_CERT "./source/server-certificate1.der"
#define CA_CERT "./source/ca-certificate.der"
#define SERVER_KEY "./source/server-key1.der"
*/
...
int main(void)
{
   ...
   /* ret = mbedtls_x509_crt_parse_file(&srvcert, SERVER_CERT);
    ...
    ret = mbedtls_x509_crt_parse_file(&srvcert, CA_CERT);
    if (ret != 0) {
        mbedtls_printf(" failed\n  !  mbedtls_x509_crt_parse returned %d\n\n", ret);
        goto exit;
    }/

    ret =mbedtls_pk_parse_keyfile(&pkey, SERVER_KEY, NULL,NULL,NULL);
    if (ret != 0) {
    ...
    */
    s_bind(&listen_fd, NULL, "4433", MBEDTLS_NET_PROTO_TCP);

    mbedtls_printf(" ok\n");
   
   /* int s_accept(mbedtls_net_context * bind_ctx,mbedtls_net_context * client_ctx,void * client_ip,size_t buf_size,size_t * ip_len,struct security_config certificates)
 */
    s_accept(&ssl,&listen_fd, &client_fd, NULL, 0, NULL,certificates,&conf,&entropy,&ctr_drbg);
...
mbedtls_x509_crt_info(cn_buf, sizeof(cn_buf), "CN",&srvcert );
printf("%s",cn_buf);
